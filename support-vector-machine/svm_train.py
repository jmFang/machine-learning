...
def showSVM(svm):
    ...
    dis = round(2/w.T.dot(w)[0,0], 2)
    print("max margin: ", dis)
    min_x = min(svm.train_x[:, 0])[0, 0]
    max_x = max(svm.train_x[:, 0])[0, 0]
    y_min_x = float(-svm.b - w[0] * min_x) / w[1]
    y_max_x = float(-svm.b - w[0] * max_x) / w[1]
    plt.plot([min_x, max_x], [y_min_x, y_max_x], '-g', label='divide line')
    plt.legend()
    plt.title("b:" + str(svm.b[0,0]) + ", w: [" + str(w[0,0]) + ", " + str(w[1,0]) + "], " + "margin:" + str(dis))
    plt.show()